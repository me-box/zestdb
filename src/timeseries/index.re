...
let add_tuple tup lis => {
  List.cons tup lis |> tup_sort;
};

let tuple_exists tup lis => {
  List.exists (fun t => t == tup) lis;
};

let since t1 lis => {
  List.filter (fun (t1',t2') => t1 <= t1') lis;
};

let until t2 lis => {
  List.filter (fun (t1',t2') => t2 >= t1') lis;
};

let range (t1,t2) lis => {
  since t1 lis |> until t2;
};
...
let bounds lis => {
  if (lis == []) {
    ...
    let xs = List.map (fun (x,_) => x) lis |> Array.of_list;
    let ys = List.map (fun (_,y) => y) lis |> Array.of_list;
...
let filter_list rem_lis lis => {
  ...
  let rec loop acc l => {
    ...
    | [] => acc;
    | [x, ...xs] => 
      (mem x rem_lis) ? loop acc xs : loop (cons x acc) xs;
    };
  };
  loop [] lis;  
};
...
let overlap_worker index lis => {
  let (x, y) = index;
  List.filter (fun (x', y') => x <= y' && y >= x') lis;
};
